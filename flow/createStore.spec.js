...
import { createStore, StoreValue, Store } from 'global-store'
...
test('create with same id will get existing store', () => {
  ...
  const store1: Store<{ a: number }> = createStore({ moduleName, key, version, initializer })
  const store2: Store<{ a: number }> = createStore({ moduleName, key, version, initializer })
  store1.value.a = 2
...
describe('freeze store', () => {
  ...
  test('property values are frozen', () => {
    const store = createStore({ moduleName, key: 'prop-freeze', version: 0, initializer: () => ({ a: 1, b: true, c: 'str', d: {}, e: [] }) })
    store.freeze()
  ...
  test('array property is frozen', () => {
    const store = createStore({ moduleName, key: 'prop-array-freeze', version: 0, initializer: () => ({ arr: [], und: undefined }) })
    store.freeze()
